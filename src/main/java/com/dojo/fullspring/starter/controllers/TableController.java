...
public class TableController {
	...
	public String dashboards(HttpSession session,
		...
		model.addAttribute("assignedTables", tableService.getAssignedTables(user)); // toutes les tables qui st assignès
																					// a cet utilisateur specifique

		return "dashboard.jsp";
	}

	// allTable page // all tables all users and Open Table page // all tables all
	// not users servers
	 ...
	 * 
	 * 
	 * model.addAttribute("AllTablesNotServers",
	 * tableService.getNotServersTables());
	...
	public String openEditTable(@PathVariable("id") Long id,
		...
		Long cur1 = Long.valueOf(userId);
		Long cur2 = Long.valueOf(table.getServer().getId());
		if (cur1 != cur2) {
			// System.err.println("++++++++++++++++++++++"+ cur1 +" "+ cur2);
			return "redirect:/dashboard";
		}
	...
	public String deleteTables(@PathVariable("id") Long id,
		...
		Object serverId = table.getServer().getId();
		if (serverId != null) {
			Long cur2 = Long.valueOf((Long) serverId);
			Long cur1 = Long.valueOf((Long) session.getAttribute("userId"));
